...
# if SAVE_BEST_MODEL_PATH is not `None` then fit LightGBM with best hyperparameter set on full dataset
# SAVE_BEST_MODEL_PATH = None
SAVE_BEST_MODEL_PATH = "light_gbm.model"


def fit(trial, train_x, train_y, val_x, val_y, model_save_path=None):
...
def objective(trial, data, model_save_path=None):
    if model_save_path is not None:
        # use all data
        ...
        fit(trial, train_x, train_y, None, None, model_save_path)
...
if __name__ == "__main__":
    ...
    if SAVE_BEST_MODEL_PATH is None:
        ...
        objective_partial(study.best_trial, model_save_path=SAVE_BEST_MODEL_PATH)